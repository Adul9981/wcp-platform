...
def score_overlooked(t):
    """💎 最被忽视：有策略逻辑 + 低关注/新上线（先发机会，策略N）。

    重要（用户明确，2026-06-10）：**低流动性不作为剔除理由**。
    新上线事件关注度低、流动性不足是正常状态，恰是先发机会所在，不设流动性门槛。
    ...
    interest, ikind = _interest(t)   # 有趣度：顶出H2H/大洲/花边等深挖盘
    score = 0.35 * low_attention + 0.15 * strat + 0.10 * new_bonus + 0.40 * interest
    tag = "新上线" if t["is_new"] else "低关注"
    extra = f"·{ikind}" if ikind else ""
    reason = f"{tag}{extra}+策略{t['strategy_tags']}；成交仅{t['volume']:.0f}→信息差/先发机会（低流动属正常）"
    return round(score, 4), reason


# 深挖出的"被忽视但有趣"盘类型(信息差所在) → 高有趣度
_INTERESTING = {
    "球员H2H": "H2H对决", "夺冠": "", "大洲夺冠": "大洲盘", "球员参赛": "球员信息差",
}
_INTERESTING_CAT = {
    "12大洲夺冠": "大洲盘", "10大洲其他": "大洲盘", "08花边政治": "花边盘",
    "13球员参赛": "球员信息差", "07球员表现": "球员盘", "03个人奖项": "个人奖项",
